...
from utils.losses.loss_helpers import (
    probability_correct_class,
    probability_wrong_class,
    log_class_prob_at_t_plus_zt,
)
from utils.random_numbers import sample_three_uniform_numbers

MU_DEFAULT = 150.0
NB_DAYS_IN_YEAR = 365.0
...
class DailyRewardPiecewiseLinRegrLoss(nn.Module):
    def __init__(
        ...
        alpha: float = 1.0,
        weight=None,
        alpha_decay: list = None,
        epochs: int = 100,
        start_decision_head_training: int = 0,
        factor: str = "v1",
        **kwargs,
    ):
        ...
        self.mu = (
            kwargs.get("sequencelength", NB_DAYS_IN_YEAR) * MU_DEFAULT / NB_DAYS_IN_YEAR
        )
        self.mus = kwargs.get("mus", torch.ones(len(weight)) * self.mu).to(
            weight.device
        )
        self.percentage_earliness_reward = torch.tensor(
            kwargs.get("percentage_earliness_reward", 0.5), device=weight.device
        )

        # initialize the percentages of alpha_2, alpha_3 and alpha_4
        if ("percentages_other_alphas" in kwargs) and (
            kwargs["percentages_other_alphas"] is not None
        ):
            if len(kwargs["percentages_other_alphas"]) != 3:
                raise ValueError("percentages_other_alphas should have length 3.")
            if not torch.isclose(
                torch.tensor(kwargs["percentages_other_alphas"]).sum(),
                torch.tensor(1.0),
            ):
        ...
        if not isinstance(self.percentages_other_alphas, torch.Tensor):
            self.percentages_other_alphas = torch.tensor(
                self.percentages_other_alphas, device=self.weight.device
            )
    ...
    def forward(
        ...
        **kwargs,
        ...
        log_class_probabilities_at_t_plus_zt = log_class_prob_at_t_plus_zt(
            log_class_probabilities, timestamps_left
        )
        ...
        if epoch >= self.start_decision_head_training and self.alpha < 1.0 - 1e-8:
            # equation (8)
            earliness_reward = (
                probability_correct_class(
                    log_class_probabilities_at_t_plus_zt, y_true, weight=self.weight
                )
                * (1 - t / T)
                * (1 - timestamps_left.float() / T)
            )
            earliness_reward = earliness_reward.sum(1).mean(0)

            # equation (9) if "v1", or (10) if "v2"
            wrong_pred_penalty = self.compute_wrong_prediction_penalty(
                log_class_probabilities_at_t_plus_zt, timestamps_left, y_true, t, T
            )

            # equation (11)
            lin_regr_zt_loss = lin_regr_zt(
                t, T, self.mus, timestamps_left.float(), y_true
            )  # (N, T)
            lin_regr_zt_loss = lin_regr_zt_loss.sum(1).mean(
                0
            ...
            wrong_pred_penalty = torch.tensor(
                0.0, device=log_class_probabilities.device
            )
        ...
        loss = (
            self.alphas[0] * classification_loss
            - self.alphas[1] * earliness_reward
            + self.alphas[2] * wrong_pred_penalty
            + self.alphas[3] * lin_regr_zt_loss
        )
    ...
    def compute_wrong_prediction_penalty(
        self, log_class_probabilities_at_t_plus_zt, timestamps_left, y_true, t, T
    ):
        ...
        """
        if self.factor == "v1":
            factor = (1.0 - t / T) * (1.0 - timestamps_left.float() / T)
        elif self.factor == "v2":
        ...
        wrong_pred_penalty = wrong_pred_penalty.sum(1).mean(
            0
        )  # sum over time, mean over batch
    ...
    def update_alphas_at_epoch(self, epoch: int, device: torch.device) -> None:
        ...
        if hasattr(self, "alpha_decay_max"):
            if epoch >= self.start_decision_head_training:
                ...
                self.alpha = self.alpha_decay_min + (
                    self.alpha_decay_max - self.alpha_decay_min
                ) * (
                    1
                    - (epoch - self.start_decision_head_training)
                    / (self.epochs - self.start_decision_head_training)
                )
    ...
    def update_alphas(self, alpha: float, device: torch.device) -> None:
        ...
        """
        alphas = torch.tensor(
            [alpha, *self.percentages_other_alphas * (1.0 - alpha)],
            device=self.weight.device,
        )
        assert torch.isclose(
            alphas.sum(), torch.tensor(1.0, device=device)
