...
def log_class_prob_at_t_plus_zt(log_class_probabilities, timestamps_left):
    ...
    """
    t_plus_zt = (
        torch.arange(log_class_probabilities.shape[1])
        .unsqueeze(0)
        .to(log_class_probabilities.device)
        + timestamps_left.int()
    )  # shape (batchsize, sequencelength)
    # if t_plus_zt is larger than the last index, set it to the last index
    t_plus_zt = torch.min(
        t_plus_zt,
        torch.tensor(log_class_probabilities.shape[1] - 1).to(t_plus_zt.device),
    )
    result = log_class_probabilities[
        torch.arange(log_class_probabilities.shape[0]).unsqueeze(1), t_plus_zt, :
