...
def create_results_table(
    ...
    for run_name in run_names:
        ...
        model_path = os.path.join(
            model_path, partition + "_" + str(sequencelength_test)
        )
        ...
        if os.path.exists(test_stats_path):
            ...
            model, criterion = get_loaded_model_and_criterion(
                run, nclasses, input_dim, mus=mus
            )

            # ## Test the model on the test dataset
            test_stats, stats = get_test_stats_from_model(
                model, test_ds, criterion, run_config
            )
    ...
    results_table = {
        run_name: {
            key: value for key, value in test_stats.items() if key in metrics_to_keep
        }
        for run_name, test_stats in results_table.items()
    }
...
def get_latex_table(
    ...
    df = df[
        ["Wrong pred. penalty"]
        + [col for col in df.columns if col != "Wrong pred. penalty"]
    ]
