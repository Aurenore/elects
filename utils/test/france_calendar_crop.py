...
def add_crop_calendar(ax, labels_names, shift=-0.3):
    ...
    columns_date = [
        ...
        "Mid-Season Start",
        "Mid-Season End",
        "Second Mid-Season Start",
        "Second Mid-Season End",
        "Harvest Start",
        "Harvest End",
    ]
    df[columns_date] = df[columns_date].map(
        lambda x: month_to_day_of_year(x, 2017) if x != 0 else 0
    )
    width = 0.6
    ...
    for i, y_label in enumerate(labels_names):
        for crop_label in df["Crop"].values:
            if crop_label in y_label:
                ...
                row = df[df["Crop"] == crop_label].iloc[
                    0
                ]  # Ensure only one row is indexed
                # Plant phase
                ax.add_patch(
                    plt.Rectangle(
                        (row["Plant Start"], yticks[i] + shift),
                        row["Plant End"] - row["Plant Start"],
                        width,
                        color=colors_calendar[0],
                        label="Plant" if not label_printed else "",
                    )
                )
                # Mid-Season phase
                ax.add_patch(
                    plt.Rectangle(
                        (row["Mid-Season Start"], yticks[i] + shift),
                        row["Mid-Season End"] - row["Mid-Season Start"],
                        width,
                        color=colors_calendar[1],
                        label="Mid-Season" if not label_printed else "",
                    )
                )
                ax.add_patch(
                    plt.Rectangle(
                        (row["Second Mid-Season Start"], yticks[i] + shift),
                        row["Second Mid-Season End"] - row["Second Mid-Season Start"],
                        width,
                        color=colors_calendar[1],
                    )
                )
                ...
                ax.add_patch(
                    plt.Rectangle(
                        (row["Harvest Start"], yticks[i] + shift),
                        row["Harvest End"] - row["Harvest Start"],
                        width,
                        color=colors_calendar[2],
                        label="Harvest" if not label_printed else "",
                    )
                )
                label_printed = True
