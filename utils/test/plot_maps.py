...
def save_patch_legend(storepath, classes_cmap, labels, file_sufix="", extension="png"):
    ...
    cmap = plt.get_cmap(classes_cmap, num_labels)  # Get discrete colors from the colormap
...
def plot_map_correct(fig, ax, annot_fields, polygon, storepath, extension):
    ...
    fig.savefig(filename, bbox_inches="tight",transparent=True)
    save_patch_legend(storepath, newcmp, ["incorrect", "correct"], file_sufix="_correct", extension=extension)
    print("figure saved at ", filename)
    
def plot_map_predictions_at_t_stop(fig, ax, annot_fields, polygon, classes_cmap, storepath, extension, class_names, vmin=VMIN, vmax=VMAX):
    xmin, ymin, xmax, ymax = polygon.geometry.total_bounds
    ax.set_xlim(xmin, xmax)
    ax.set_ylim(ymin, ymax)
    ax.axis("off")
    ...
    fig.savefig(filename, bbox_inches="tight",transparent=True)
    save_patch_legend(storepath, classes_cmap, class_names, file_sufix="_predictions", extension=extension)
    print("figure saved at ", filename)
...
def plot_map_stop_date(fig, ax, annot_fields, date, polygon, storepath, stopped_cmap, extension="png"):
    ...
    fig.savefig(filename, bbox_inches="tight",transparent=True)
    save_patch_legend(storepath, stopped_cmap, ["not stopped", "stopped"], file_sufix="_stopped", extension=extension)
    print("figure saved at ", filename)
...
def plot_map_table(storepath, dates_of_interest, sat_pics_path, extension="png", fontsize=18):
    ...
    categories = {
        ...
        'Prediction': ['prediction_at_date_{}.png'.format(date) for date in dates] + ["predictions.png"],
        'Correct/Incorrect': ['correct_at_date_{}.png'.format(date) for date in dates] + ["correct.png"],
        'Stopped': ['stopped_at_date_{}.png'.format(date) for date in dates] + ['doy_stop.png']
    }
    last_col_labels = ['Ground Truth', 'Prediction at stop', 'In/correct at stop', 'Day of Stopping']

    # Initialize the plot
    fig = plt.figure(figsize=(18, 16))
    gs = GridSpec(4, 5, figure=fig, width_ratios=[1, 1, 1, 1, 0.5])  # Reduce the last column width
    ...
    for row, (label, files) in enumerate(categories.items()):
        fig.text(0.005, 0.88 - (row / 4), label, va='center', ha='left', fontsize=fontsize, \
        ...
        for col, file in enumerate(files):
            if row==0 and col<len(dates): 
                ...
                if img is not None:
                    axs[row][col].imshow(img)
                    if col==3:
                        axs[row][col].set_title('{}'.format(last_col_labels[row]), fontsize=fontsize)
                        
    # place legends in the last column 
    legend_files = ["legend_predictions.png","legend_correct.png","legend_stopped.png"]
