...
def plot_one_parcel_prediction(
    class_names,
    ...
    ax.text(
        t_stop + 3.0, ymax - 10.0, f"stop: {t_stop.item()}", verticalalignment="center"
    )
    fig.tight_layout()
    # save the figure
...
def plot_spectral_bands(X, fig, ax, model_path):
    ...
    for i in range(13):
        ax.plot(
            idx, X[X[:, i] != 0, i], label=f"{SELECTED_BANDS['L1C'][i]}", linewidth=0.5
        )
