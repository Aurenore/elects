...
def main_train(config):
    ...
    with tqdm(range(start_epoch, config.epochs + 1)) as pbar:
        for epoch in pbar:
            ...
            # get metrics
            dict_results_epoch, train_stats = get_all_metrics(
                stats,
                config,
                epoch,
                train_stats,
            ...
            # plot metrics
            dict_results_epoch = plots_during_training(
                epoch,
                stats,
                config,
                dict_results_epoch,
            ...
            savemsg, not_improved = update_patience(
                df, testloss, config, model, optimizer, not_improved
            )

            # log description
            log_description(
                pbar, epoch, trainloss, testloss, dict_results_epoch, savemsg
            )
            ...
            if config.patience is not None:
                if not_improved > config.patience:
                    print(
                        f"stopping training. testloss {testloss:.2f} did not improve in {config.patience} epochs."
                    )
                    break
