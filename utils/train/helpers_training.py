...
def load_dataset(config, partition="train"):
    ...
    if config.dataset == "breizhcrops":
        ...
        doys_dict_test = get_doys_dict_test(
            dataroot=os.path.join(config.dataroot, config.dataset)
        )
        ...
        train_ds = BreizhCrops(
            ...
            daily_timestamps=config.daily_timestamps,
            original_time_serie_lengths=config.original_time_serie_lengths,
        )
        test_ds = BreizhCrops(
            root=dataroot,
            partition=config.validation_set,
            sequencelength=config.sequencelength,
            corrected=config.corrected,
            daily_timestamps=config.daily_timestamps,
            original_time_serie_lengths=config.original_time_serie_lengths,
    ...
    if config.decision_head == "day":
        dict_model = {
            "start_decision_head_training": (
                ...
                if hasattr(config, "start_decision_head_training")
                else 0
            ),
        }
        model = DailyEarlyRNN(
            config.backbonemodel,
            nclasses=nclasses,
            input_dim=input_dim,
            sequencelength=config.sequencelength,
            hidden_dims=config.hidden_dims,
            day_head_init_bias=config.day_head_init_bias,
            **dict_model,
        ).to(config.device)
    else:
        model = EarlyRNN(
...
def set_up_optimizer(config, model):
    ...
    optimizer = torch.optim.AdamW(
        [
            {"params": no_decay, "weight_decay": 0, "lr": config.learning_rate},
            {"params": decay},
        ],
...
def set_up_class_weights(config, train_ds):
    ...
    config.update(
        {
            "class_weights": (
                class_weights.cpu().detach().numpy()
...
def set_up_criterion(
    config, class_weights, nclasses, mus: torch.tensor = None, wandb_update: bool = True
):
    ...
    if config.loss == "early_reward":
        criterion = EarlyRewardLoss(
            alpha=config.alpha, epsilon=config.epsilon, weight=class_weights
        )
    elif config.loss == "daily_reward_piecewise_lin_regr":
        dict_criterion = {
            "mus": mus,
            "percentages_other_alphas": (
                config.percentages_other_alphas
                if hasattr(config, "percentages_other_alphas")
                else None
            ),
        }
        ...
        if wandb_update:
            wandb.config.update(
                {
                    "percentages_other_alphas": criterion.percentages_other_alphas.cpu()
                    .detach()
                    .numpy()
                }
            )
            wandb.config.update(
                {
                    "percentage_alpha_1": criterion.percentages_other_alphas[0]
                    .cpu()
                    .detach()
                    .numpy()
                }
            ...
            wandb.config.update(
                {
                    "percentage_alpha_2": criterion.percentages_other_alphas[1]
                    .cpu()
                    .detach()
                    .numpy()
                }
            ...
            wandb.config.update(
                {
                    "percentage_alpha_3": criterion.percentages_other_alphas[2]
                    .cpu()
                    .detach()
                    .numpy()
                }
...
def set_up_resume(config, model, optimizer):
    ...
    if config.resume and os.path.exists(config.snapshot):
        ...
        optimizer_snapshot = os.path.join(
            os.path.dirname(config.snapshot),
            os.path.basename(config.snapshot).replace(".pth", "_optimizer.pth"),
        )
        optimizer.load_state_dict(
            torch.load(optimizer_snapshot, map_location=config.device)
        )
...
def update_dict_result_epoch(
    dict_results_epoch, stats, config, epoch, trainloss, testloss, criterion
):
...
def second_update_dict_result_epoch(
    dict_results_epoch, stats, trainloss, testloss, criterion, class_names, mus
):
...
def get_all_metrics(
    stats, config, epoch, train_stats, trainloss, testloss, criterion, class_names, mus
):
...
def plots_during_training(
    ...
    if epoch % 5 == 0 or epoch == 1 or epoch == config.epochs:
        ...
        if config.daily_timestamps:
            ...
            doys_dict = get_approximated_doys_dict(
                stats["seqlengths"], length_sorted_doy_dict_test
            )
            doys_stop = get_doy_stop(stats, doys_dict)
        fig_boxplot, _ = boxplot_stopping_times(
            doys_stop, stats, fig_boxplot, ax_boxplot, class_names, epoch=epoch
        )
        ...
        if "lin_regr" in config.loss:
            ...
            dict_results_epoch["class_probabilities_wrt_time"] = wandb.Image(
                fig_prob_class
            )
