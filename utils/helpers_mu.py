...
def extract_mu_thresh(class_prob, y_true, p_tresh, mu_default):
    ...
    for label in range(nb_classes):
        mean_prob = class_prob[y_true == label].mean(
            axis=0
        )  # shape (n_days, nb_classes)
...
def mus_should_be_updated(config, epoch, freq_update=5):
    ...
    """
    if (
        "lin_regr" in config.loss
        and epoch >= config.start_decision_head_training
        and epoch % freq_update == 0
    ):
        return True
    else:
        return False
