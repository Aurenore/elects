...
def load_personal_config(
    path: str = os.path.join("..", "config", "personal_config.yaml")
):
...
def load_personal_wandb_config(
    path: str = os.path.join("..", "config", "personal_config.yaml")
):
    ...
    assert (
        len(entity) > 0
    ), "entity should not be empty, please fill in the personal_config.yaml file"
    assert (
        len(project) > 0
    ), "project should not be empty, please fill in the personal_config.yaml file"
    if len(sweep) == 0:
...
def set_up_config(config, print_comments: bool = False, final_train: bool = False):
    ...
    if not hasattr(config, "original_time_serie_lengths"):
        ...
        if print_comments:
            print(
                f"original_time_serie_lengths set to {config.original_time_serie_lengths}"
            )
