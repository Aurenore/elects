...
from utils.helpers_config import (
    load_personal_wandb_config,
    set_up_config,
    save_config,
    print_config,
)
from utils.test.helpers_testing import (
    get_test_stats_from_model,
    load_test_dataset,
    save_test_stats,
)
...
from utils.results_analysis.extract_video import (
    download_images,
    add_files_to_images,
    save_video,
)
...
def main(
    ...
    entity, project, _ = load_personal_wandb_config(
        os.path.join("config", "personal_config.yaml")
    )
    ...
    test_ds, nclasses, class_names, input_dim = load_test_dataset(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--sequencelength-test",
        type=int,
        help="sequence length of the test dataset",
        default=None,
    )
    parser.add_argument(
        "--plot-label-distribution",
        type=bool,
        help="if true, plot the label distribution",
        default=False,
    )
    parser.add_argument(
        "--dataroot",
        type=str,
        help='local dataroot. If default, then local dataroot will be set to os.path.join(os.environ.get("HOME", os.environ.get("USERPROFILE")),"elects_data")',
        default="default",
    )
    parser.add_argument(
        "--partition",
        type=str,
        help="partition to test on, should be 'train', 'valid', or 'eval'",
        default="eval",
    )
