...
def test_probability_wrong_class():
    ...
    logprobabilities = torch.log(
        torch.rand(batchsize, sequencelength, nclasses) + 1e-6
    )  # Avoid log(0)
    ...
    logprobabilities[:, :, 0] = math.log(1.0)
    ...
    assert torch.allclose(
        output, torch.zeros(batchsize, sequencelength)
    ), "Output should be zero, with log(1) for the first class and targets, weight=None."

    output = probability_wrong_class(logprobabilities, targets, weight)
    assert torch.allclose(
        output, torch.zeros(batchsize, sequencelength)
    ), "Output should be zero, with log(1) for the first class and targets"
    ...
    assert torch.allclose(
        output, torch.ones(batchsize, sequencelength)
    ...
    ), "Output should be the weights of the classes, with shape (batchsize, sequencelength)."
