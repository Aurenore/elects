...
from utils.helpers_mu import (
    mus_should_be_updated,
    update_mus_during_training,
    extract_mu_thresh,
)
...
class TestMus:
    ...
    def test_update_mus(self):
        ...
        for epoch in range(1, 100):
            assert mus_should_be_updated(config, epoch, freq_update=5) == (
                epoch >= config.start_decision_head_training and epoch % 5 == 0
            )
    ...
    def test_extract_mu_tresh(self):
        ...
        assert (
            len(mus) == nb_classes
        ), "Mus should have the same length as the number of classes."
