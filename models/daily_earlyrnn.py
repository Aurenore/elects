...
class DailyEarlyRNN(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.stopping_decision_head = DecisionHeadDay(
            hidden_dims, day_head_init_bias, sequencelength
        )
        ...
        if "start_decision_head_training" in kwargs:
            self.start_decision_head_training = kwargs.get(
                "start_decision_head_training", 0
            )
        else:
    ...
    def forward(self, x, **kwargs):
        ...
        if epoch >= self.start_decision_head_training and alpha < 1.0 - 1e-8:
            timestamps_left_before_predictions = self.stopping_decision_head(outputs)
        else:
            timestamps_left_before_predictions = (
